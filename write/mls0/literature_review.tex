...
% <><><><><<
% Models might also suffer undesirable three-dimensional effects, where radar profiles may not perfectly align with surface velocity fields, effectively neglecting real transverse ice dynamical effects. 

% can you expand on this? i'm not sure i fully understand what you want to convey here, particularly how the radar data are currently used to constrain ice dynamics and what that means for areas where we have data gaps
% >><><><><>

% Model uncertainty increases in regions with variable bed conditions
% <><><><><<EXPLAIN WHY>><><><><>
...
%%%this page needs a bit more context so that readers understand what's going on. try and step through a bit more slowly and describe things in a bit more detail. 

% e.g., i'd recommend making sure you have sentences in the different sections that link back to the previous section. in the inversion section, make sure you describe what inversion is and how it relates to the ssa equations, before describing what it assumes. then with the transfer functions, describe how they relate to the previous two paragraphs
...
\subsection*{Theoretical Framework}
 ...
 % make sure you describe what inversion is and how it relates to the ssa equations, before describing what it assumes. then with the transfer functions, describe how they relate to the previous two paragraphs
...
% The SSA describes relationship between ice flow and bed conditions:
% \begin{equation}\partial_{x} (4 h \eta \partial_{x} u + 2 h \eta \partial_y v) + \partial_{y}(h \eta( \partial_{x} v + \partial_{y} u)) - (u/c^{1/m}) = \rho g h ( \partial_{x} (s) \mathrm{cos}(\alpha) - \mathrm{sin}(\alpha))
...
% Equations \ref{eq:2.1} and \ref{eq:2.2} describe a linearised system around a reference model $(\bar{h}, \bar{s}, \bar{b}, \bar{u}, \bar{v}, \bar{c})$, leading to first-order momentum balance equations:
% \begin{equation}
